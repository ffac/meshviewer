define(["moment", "numeral", "tablesort", "c3", "d3", "tablesort.numeric"],
  function (moment, numeral, Tablesort, c3, d3) {
  ...
  function showChart(chartConfig, d) {
    ...
    var zoom = {
      ...
      "buttons": [],
      "config": null,
      "enabled": chartConfig.zoom
                  && chartConfig.zoom.enabled
                  && chartConfig.zoom.levels
                  && chartConfig.zoom.levels.length > 0
    }

    var div = document.createElement("div")
    div.classList.add("chart")
    var h4 = document.createElement("h4")
    h4.textContent = chartConfig.name
    div.appendChild(h4)

    if (!chartConfig.data.parse)
      chartConfig.data.parse = function(results) {
        var data = []
          ...
          if (d.results && d.results[0] && d.results[0].series && d.results[0].series[0])
            d.results[0].series[0].values.forEach(function (vs) {
              ...
              vs.forEach(function (v, k) {
                ...
                if (chartConfig.data.format && chartConfig.data.format[target])
                  v = (typeof chartConfig.data.format[target] === "function" ?
                    chartConfig.data.format[target](v, zoom) :
                    d3.format(chartConfig.data.format[target])(v))
    ...
    if (zoom.enabled) {
      zoom.level = 0
      zoom.config = chartConfig.zoom.levels[0]
    }

    if (!chartConfig.c3.data)
      chartConfig.c3.data = {}
    if (!chartConfig.c3.legend)
      chartConfig.c3.legend = {}

    // Toggle on legend click
    if (!chartConfig.c3.legend.item || !chartConfig.c3.legend.item.onclick) {
      if (!chartConfig.c3.legend.item)
        chartConfig.c3.legend.item = {}
      chartConfig.c3.legend.item.onclick = function(id) {
        if (chartConfig.data.toggle)
          chartConfig.data.toggle.call(this, id)
        this.api.hide()
        this.api.show(id)
      }
    }
    ...
    if (chartConfig.c3.axis
      && chartConfig.c3.axis.x
      && chartConfig.c3.axis.x.tick
      && chartConfig.c3.axis.x.tick.format) {
      var tf = chartConfig.c3.axis.x.tick.format
      if (typeof tf === "function")
        chartConfig.c3.axis.x.tick.format = function(val) {
          return tf(val, zoom)
        }
    }

    var getData = function(callback) {
      var cacheKey = d.nodeinfo.node_id + "_" + zoom.level
      if (cache[cacheKey])
        return callback(cache[cacheKey])
      else {
        var urls = (typeof chartConfig.data.urls === "function" ? chartConfig.data.urls(d, zoom) : chartConfig.data.urls)
        Promise.all(urls.map(getJSON)).then(function(data) {
          data = chartConfig.data.parse ? chartConfig.data.parse(data, zoom) : data
          cache[cacheKey] = data
          callback(data)
        })
      }
    ...
    var updateChart = function(data) {
      chartConfig.c3.data.json = data
      chart.load(chartConfig.c3.data)
    }

    var drawChart = function(data) {

      chartConfig.c3.data.json = data
      chart = c3.generate(chartConfig.c3)
      ...
      if (zoom.enabled) {
        ...
        chartConfig.zoom.levels.forEach(function(v, level) {
