define(["moment", "numeral", "tablesort", "c3", "d3", "tablesort.numeric"],
  ...
  function showChart(chartConfig, d) {
    ...
    if (!chartConfig.data.parse)
      chartConfig.data.parse = function (d) {
        var data = []
            ...
            vs.forEach(function(v, k) {
              ...
              if (chartConfig.data.format && chartConfig.data.format[target])
                v = (typeof chartConfig.data.format[target] === "function" ?
                  chartConfig.data.format[target](v) :
    ...
    var drawChart = function(data) {
      ...
      if (chartConfig.data.load)
        data = chartConfig.data.load(data)
      ...
      if (!chartConfig.c3.legend)
        ...
        chartConfig.c3.data.json = (chartConfig.data.parse ? chartConfig.data.parse(data) : data)
      var chart = c3.generate(chartConfig.c3)
      div.appendChild(chart.element)
    }

    if (chartConfig.data.url)
      getJSON(chartConfig.data.url(d)).then(function (data) {
        drawChart(data)
      })
    else drawChart()
